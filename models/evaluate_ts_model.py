...
class PIDNet(nn.Module):
	...
	def forward(self, x):
		...
		height_output = math.ceil(x.shape[-2] /cons)
		# print("width_output, height_output: ", width_output, height_output)
...
def check_trt_model(model, model_path):
		try:
			if not os.path.exists(model_path):
				print("ts file not found. compiling a new ts file")

				# create tensorrt module
				traced_model = torch.jit.trace(model, torch.empty([1, 3, 540, 960]).to("cuda"))
				trt_ts_module = torch_tensorrt.compile(traced_model, inputs=[torch_tensorrt.Input(
								min_shape=[1, 3, 540, 960],
								opt_shape=[1, 3, 540, 960],
								max_shape=[1, 3, 540, 960],
								dtype=torch.half
								)],
...
if __name__ == '__main__':
	...
	model = get_pred_model(name='pidnet_m', num_classes=10)
	model.eval()
	model.to(device)
	iterations = None
	model_path = 'test_1080_1920.ts'
	model_trt = check_trt_model(model, model_path)

	input = torch.randn(1, 3, 1080, 1920).cuda()
	with torch.no_grad():
		for _ in range(10):
			model_trt(input.half()) #model_trt(input.half())
		if iterations is None:
			...
			while elapsed_time < 1:
				...
				for _ in range(iterations):
					model_trt(input.half()) #model(input) #
				torch.cuda.synchronize()
				torch.cuda.synchronize()
		...
		t_start = time.time()
		for _ in range(iterations):
			pred = model_trt(input.half()) #model(input) #
		torch.cuda.synchronize()
		torch.cuda.synchronize()
...
################## My laptop ###################
# loading saved tensorrt model file
# input: 1080x1920
# WARNING: [Torch-TensorRT] - TensorRT was linked against cuBLAS/cuBLASLt 11.6.5 but loaded cuBLAS/cuBLASLt 11.4.2
# WARNING: [Torch-TensorRT] - TensorRT was linked against cuBLAS/cuBLASLt 11.6.5 but loaded cuBLAS/cuBLASLt 11.4.2
# =========Speed Testing=========
# pred:  torch.Size([1, 19, 135, 240])
# 58.53104310262285
